...
pub async fn local_state_query_server_and_block_query_get_epoch_client_happy_path() {
    let server = tokio::spawn({
        // TODO: create a common function to setup the server
        async move {
            // server setup
            let socket_path = Path::new("node.socket");
...
#[tokio::test]
pub async fn local_state_query_server_and_block_query_get_stake_distribution_client_happy_path() {
    let server = tokio::spawn({
        async move {
            // server setup
            let socket_path = Path::new("node.socket");
            ...
            assert_eq!(
                query,
                Request::BlockQuery(localstate::queries::BlockQuery::GetStakeDistribution)
            );
            ...
            server_sq
                .send_result(GenericResponse::new(
                    hex::decode("8B188118BF1858181C031218EA188E183D18E11821").unwrap(),
                ))
                .await
                .unwrap();
